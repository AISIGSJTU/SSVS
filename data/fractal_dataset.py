...
import numpy as np
import torch
from data.data_fractals import fractal
...
class FRACTALDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        A = fractal()#self.transform_A(A_img)
        #A = (A_img/ 255.0).astype(np.uint8)
        #A = torch.from_numpy(np.expand_dims(A, 0)).float()
        return {'A': A, 'B': B, 'C': C, 'A_paths': C_path, 'B_paths': B_path,'C_paths': C_path}


    def __len__(self):
        """Return the total number of images."""
        return max(self.C_size, self.B_size)
