...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[],thres=False):
    """Create a generator
...
class hybirdloss:
  def __init__(self, c):
      self.seg_loss = nn.BCEWithLogitsLoss() #pos_weight=torch.tensor([4])
      self.c=c

  def __call__(self, img_pred, img_true):
      img_true=(img_true+1)/2
      img_pred=(img_pred+1)/2
      L = (1 - self.c) * self.seg_loss(img_pred, img_true)
      eps = 1e-15
      jaccard_target = (img_true == 1).float()
      jaccard_output = (img_pred == 1).float()#nn.functional.sigmoid(img_pred)
      intersection = (jaccard_output * jaccard_target).sum()
      union = jaccard_output.sum() + jaccard_target.sum()
      L -=self.c * torch.log((intersection + eps) / (union - intersection + eps))
